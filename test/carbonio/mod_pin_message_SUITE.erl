...
setup(Config, pin_message_basic_test) ->
    ...
    meck:expect(mod_pin_message_backend, write, fun(_HostType, _Pin) -> ok end),
    meck:expect(mod_pin_message_backend,
                read,
                fun(_HostTypem, <<"CHFOU8II7901">>) ->
                   {ok,
                    {pin,
                     1,
                     <<"pin1">>,
    ...
    [{acc, default_accumulator()} | Config];
setup(Config, get_pin_with_existing_pin_test) ->
    ...
    meck:expect(mod_pin_message_backend,
                get_pin_by_room_id,
                fun(_HostTypem, <<"1765-785258-62595@muclight.localhost">>) ->
                   {ok,
                    {pin,
                     1,
                     <<"pin1">>,
    ...
    [{acc, get_pin_accumulator()} | Config];
setup(Config, pin_message_with_not_existing_message_test) ->
    ...
    meck:expect(mod_pin_message_backend,
                read,
                fun(_HostTypem, <<"CHFOU8II7901">>) -> error end),
    meck:new(ejabberd_router, [passthrough]),
    meck:expect(ejabberd_router,
                route,
                fun(_From,
...
setup(Config, get_pin_with_not_existing_pin_test) ->
    ...
    meck:expect(mod_pin_message_backend, check_message_exists, fun(_, _) -> false end),
    meck:expect(mod_pin_message_backend, init, fun(_HostType, _Opts) -> ok end),
    meck:expect(mod_pin_message_backend, get_pin_by_room_id, fun(_HostTypem, _) -> error end),
    meck:new(ejabberd_router, [passthrough]),
    meck:expect(ejabberd_router,
                route,
                fun(_From,
...
setup(Config, delete_pin_test) ->
    ...
    [{acc, delete_pin_accumulator()} | Config];
                     ...
                     [{xmlel,
                       ...
                       [{xmlel, <<"operation">>, #{}, [{xmlcdata, <<"messagePinned">>, escaped}]},
                        {xmlel,
                         ...
                         #{<<"xmlns">> := <<"zextras:iq:pin">>,
                           <<"message-id">> := <<"CHFOU8II7901">>},
                         [{xmlel, <<"pinned-by">>, #{}, [{xmlcdata, <<"bob@localhost">>, escaped}]},
                          {xmlel, <<"room-id">>, #{}, [{xmlcdata, _, escaped}]},
                          {xmlel, <<"pinned-at">>, #{}, [{xmlcdata, _, escaped}]}]}]},
                      {xmlel, <<"body">>, #{}, []}]}) ->
                   ok
                end),
    [{acc, update_pin_accumulator()} | Config];
...
update_not_message_pinned_test(Config) ->
    ...
    Extra =
        #{host_type => <<"localhost">>,
          ...
          hook_tag => <<"localhost">>},
    ?assertEqual({ok, Acc},
                 mod_pin_message:pin_message(Acc, Params, Extra)).% %% Test: Unpin a message
     ...
     #{timestamp => 1767793003247273,
       ...
       stanza =>
           #{name => <<"iq">>,
             ...
             from_jid =>
                 {jid, <<"alice">>, <<"localhost">>, <<"1767-792995-821901-fce4263fa3d7301d">>}},
       non_strippable => [{c2s, module}, {c2s, origin_sid}, {c2s, origin_jid}],
       origin_location =>
           #{line => 889,
             file =>
                 "/Users/riccardodegan/work/erlang/carbonio-mongooseim/src/c2s/mongoos"
                 "e_c2s.erl",
...
get_pin_accumulator() ->
    {{jid, <<"bob">>, <<"localhost">>, <<"1767-958798-795846-78d63f7307200a67">>},
     ...
     #{timestamp => 1767958800975498,
           ...
           #{name => <<"iq">>,
             ...
             element =>
                 {xmlel,
                  <<"iq">>,
                  #{<<"id">> => <<"pin1">>,
                    <<"to">> => <<"1765-785258-62595@muclight.localhost">>,
                    <<"type">> => <<"get">>,
                    <<"xmlns">> => <<"jabber:client">>},
                  [{xmlel,
                    <<"pin">>,
                    #{<<"message-id">> => <<"CHFOU8II7901">>, <<"xmlns">> => <<"zextras:iq:pin">>},
                    []}]},
             ref => undefined,
             to_jid => {jid, <<"1765-785258-62595">>, <<"muclight.localhost">>, <<>>},
             from_jid =>
                 {jid, <<"bob">>, <<"localhost">>, <<"1767-958798-795846-78d63f7307200a67">>}},
       non_strippable => [{c2s, module}, {c2s, origin_sid}, {c2s, origin_jid}],
       origin_location =>
       ...
       {c2s, origin_jid} =>
           {jid, <<"bob">>, <<"localhost">>, <<"1767-958798-795846-78d63f7307200a67">>},
       {c2s, origin_sid} => {1767958798707588, undefined}},
     {xmlel,
      <<"iq">>,
      #{<<"id">> => <<"pin1">>,
        <<"to">> => <<"1765-785258-62595@muclight.localhost">>,
...
delete_pin_accumulator() ->
    {{jid, <<"bob">>, <<"localhost">>, <<"1767-974173-673680-d501f62252d1800f">>},
     ...
     #{timestamp => 1767974176713883,
       ...
       {c2s, origin_jid} =>
           {jid, <<"bob">>, <<"localhost">>, <<"1767-974173-673680-d501f62252d1800f">>},
       {c2s, origin_sid} => {1767974173620304, undefined}},
     {xmlel,
      <<"iq">>,
      #{<<"id">> => <<"pin1">>,
        <<"to">> => <<"1765-785258-62595@muclight.localhost">>,
    ...
    {{jid, <<"1765-785258-62595">>, <<"muclight.localhost">>, <<"alice@localhost">>},
     ...
     #{timestamp => 1768292063873840,
       ...
       stanza =>
           #{name => <<"message">>,
             ...
             element =>
                 {xmlel,
                  <<"message">>,
                  #{<<"from">> => <<"1765-785258-62595@muclight.localhost/alice@localhost">>,
                    <<"id">> => <<"533aeb61-97a4-4f95-bef0-cbbc19215ae6">>,
                    <<"to">> => <<"bob@localhost">>,
                    <<"type">> => <<"groupchat">>},
                  [{xmlel,
                    <<"apply-to">>,
                    #{<<"id">> => <<"CHFOU8II7901">>, <<"xmlns">> => <<"urn:xmpp:fasten:0">>},
                    [{xmlel, <<"edit">>, #{<<"xmlns">> => <<"zextras:xmpp:edit:0">>}, []},
                     {xmlel, <<"external">>, #{<<"name">> => <<"body">>}, []}]},
                   {xmlel, <<"body">>, #{}, [{xmlcdata, <<"test messaggio modificato">>, escaped}]},
                   {xmlel,
                    <<"stanza-id">>,
                    #{<<"by">> => <<"1765-785258-62595@muclight.localhost">>,
                      <<"id">> => <<"CHFOU8II7901">>,
       ...
       origin_location =>
           #{line => 264,
             file =>
                 "/Users/riccardodegan/work/erlang/carbonio-mongooseim/src/muc_light/m"
                 "od_muc_light_room.erl",
             mfa => {mod_muc_light_room, make_handler_fun, 1}},
       origin_pid => undefined,
       statem_acc =>
           #{socket_send => [],
             c2s_data => undefined,
       ...
       {c2s, origin_sid} => {1768292047158159, undefined}},
     {xmlel,
      <<"message">>,
      #{<<"from">> => <<"1765-785258-62595@muclight.localhost/alice@localhost">>,
        <<"id">> => <<"533aeb61-97a4-4f95-bef0-cbbc19215ae6">>,
