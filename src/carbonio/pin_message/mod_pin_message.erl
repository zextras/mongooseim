...
-spec pin_message(Acc, Params, Extra) -> {ok, Acc} when
    Acc :: mongoose_hooks:filter_packet_acc(),
    Params :: map(),
    Extra :: map().
pin_message(
    {From, To, _Acc,
        {xmlel, <<"iq">>, #{<<"id">> := RequestId, <<"type">> := <<"set">>}, [
            {xmlel, <<"pin">>, #{<<"message-id">> := MessageId, <<"action">> := <<"delete">>}, _}
        ]}},
    _Params,
...
pin_message(
                ...
                {xmlel, <<"apply-to">>,
                    ...
                    [
                        {xmlel, <<"retract">>,
                            #{
                                <<"xmlns">> :=
                                    <<"urn:esl:message-retract-by-stanza-id:0">>
                            },
                            []}
                    ]}
            ]} =
    ...
    case pin:get_pin(HostType, StanzaId) of
        {ok, _Pin} ->
            handle_delete_pin_without_answer(HostType, From, To, StanzaId),
            {ok, CompleteAcc};
        error ->
            {ok, CompleteAcc}
    end;
pin_message({_, _, _, Xml} = Acc, _Params, _Extra) ->
    {ok, Acc}.
...
handle_check_message_exists(
    ...
    handle_create_pin(
        pin:check_message_exists(HostType, ConvertedStanzaId),
        HostType,
        From,
        To,
        RequestId,
...
handle_create_pin_and_send_only_message(
    HostType,
    From,
    To,
    RequestId,
...
handle_error(From, To, RequestId) ->
    ErrorSubEl =
        {xmlel, <<"error">>, #{<<"type">> => <<"modify">>}, [
            {xmlel, <<"bad-request">>, #{<<"xmlns">> => <<"urn:ietf:params:xml:ns:xmpp-stanzas">>},
                []}
        ]},
    ErrorIq =
        #iq{
...
add_iq_handlers(HostType, Opts) ->
    ...
    gen_iq_handler:add_iq_handler_for_subdomain(
        HostType,
        MUCLightSubdomainPattern,
        ?NS_PIN,
        mod_muc_iq,
...
remove_iq_handlers(HostType) ->
    MUCLightSubdomainPattern =
        gen_mod:get_module_opt(HostType, mod_muc_light, host, <<"muclight.@HOST@">>),
    gen_iq_handler:remove_iq_handler_for_subdomain(
        HostType,
        MUCLightSubdomainPattern,
        ?NS_PIN,
        mod_muc_iq
